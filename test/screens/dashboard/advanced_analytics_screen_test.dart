...
void main() {
  ...
  testWidgets('renders and displays all tabs', (tester) async {
    await tester.pumpWidget(testableWidget(const AdvancedAnalyticsScreen()));
  ...
  testWidgets('switches between tabs and displays content', (tester) async {
    ...
    printAllTexts(tester);
    // Tab 1
    expect(find.textContaining('Correlation Matrix'), findsOneWidget);
    // Tab 2
    final tabBar = find.byType(PremiumTabBar);
    final tabWidgets = find.descendant(
      of: tabBar,
    ...
    await tester.tap(tabWidgets.at(1)); // Tap Prediction
    await tester.pumpAndSettle();
    printAllTexts(tester);
    expect(find.textContaining('Predictive Analytics'), findsOneWidget);
    // Tab 3
    await tester.tap(tabWidgets.at(2)); // Tap Visuals
    await tester.pumpAndSettle();
    printAllTexts(tester);
    expect(find.textContaining('Data Visualizations'), findsOneWidget);
  });
}
